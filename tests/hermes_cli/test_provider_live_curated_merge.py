...
class TestGenericProviderLiveCuratedMerge:
    ...
    def test_validation_rejects_models_only_in_other_provider_catalogs(self):
        """Curated fallback validation stays scoped to the requested provider."""
        live = ["GLM-5.1", "glm-5"]
        curated = ["glm-5.1", "GLM-5", "glm-4.5"]
        profile = self._make_profile(live)
        ...
        with (
            patch("providers.get_provider_profile", return_value=profile),
            patch("hermes_cli.models.fetch_api_models", return_value=live),
            patch(
                "hermes_cli.auth.resolve_api_key_provider_credentials",
                return_value={"api_key": "k", "base_url": ""},
            ),
            patch.dict(
                ...
                {"openai": ["some-other-model"], "zai": curated},
            ),
        ):
