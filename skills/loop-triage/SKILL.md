...
- Treat the supplied task id as the canonical Loop root and pass its board on every call.
...
- Preserve the user's original request on the canonical root.

## Workflow

1. Read the canonical root and current graph with `loop_graph`, including nodes.
...
4. Call `delegate_task` once for the fragment with `mode="loop"`, `decompose=true`, the canonical `root_task_id`, and batch-local aliases:
...
6. As results arrive, re-read before mutating. Add new nodes with another atomic `delegate_task` fragment. Use revision-guarded `loop_graph` patches to update pending titles, replace pending parents, archive obsolete pending nodes, or resolve handoffs.
7. Report what is live, what is waiting, and any decision that genuinely needs the user. Continue evolving the graph until the root objective is satisfied.
8. When no more work is needed, re-read the graph. Only after every current sink is terminal and the unassigned canonical root is `ready`, call `kanban_complete` on the root with a concise outcome summary. If the root is still `todo`, work remains or a dependency is unresolved; do not force completion.
