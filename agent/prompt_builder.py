...
KANBAN_GUIDANCE = (
    ...
    "## Orchestrator mode\n"
    "Use `kanban_decompose(...)` for ordinary routing. For dynamic uncertainty, "
    "build a live graph with `delegate_task(mode=\"loop\", "
    "decompose=True, tasks=[...])`: give each node only a brief title, id, and "
    "dependency aliases. Creating nodes submits them immediately; the triage "
    "auto-decomposer owns specifications, routing, and optional child graphs. "
    "Revise pending nodes as results arrive; there is no separate Submit.\n\n"
    "## Details\n"
