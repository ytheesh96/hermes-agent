...
const mocks = vi.hoisted(() => {
  ...
  const defaultLoopController = (): MockLoopController => ({
    activeWorkflowId: 'wf_research',
    canvasScopeKey: 'logical-origin',
    focusedTaskId: null,
    focusRequestKey: 0,
    focusRequestKeysByWorkflow: { wf_research: 1 },
    hidden: false,
    ...
    open: true,
    positions: undefined,
    positionsByWorkflow: { wf_research: [] },
    selectedTaskDetail: null,
    selectedTaskDetailError: null,
...
describe('ContribWiring Loop session-source wiring', () => {
  ...
  it('uses the logical session key for Loop source while a runtime session is active', () => {
    ...
    const workflowPaneId = loopWorkflowPaneId('test:logical-origin', 'wf_research')
  ...
  it('does not render a visible Loop rail placeholder when the mounted session-source controller is empty', () => {
    ...
    expect(
      $layoutTree.get() && findGroupOfPane($layoutTree.get()!, loopWorkflowPaneId('test:logical-origin', 'wf_research'))
    ).toBeNull()
    expect($hiddenTreePanes.get().has('loop')).toBe(true)
  })
  ...
  it('fronts the selected workflow canvas while keeping its peer tab open', () => {
    let controller = {
      ...mocks.defaultLoopController(),
      focusRequestKey: 1,
      focusRequestKeysByWorkflow: { wf_research: 1, wf_review: 0 },
      workflowIds: ['wf_research', 'wf_review']
    }

    mocks.useLoopPanelController.mockImplementation(() => controller)
    renderController()

    const researchPaneId = loopWorkflowPaneId('test:logical-origin', 'wf_research')
    const reviewPaneId = loopWorkflowPaneId('test:logical-origin', 'wf_review')
    ...
    act(() => {
      controller = {
        ...
        focusRequestKey: 2,
        focusRequestKeysByWorkflow: { wf_research: 1, wf_review: 1 },
  ...
  it('closes the exact native workflow while preserving the hidden layout anchor', () => {
    ...
    expect(controller.onCloseWorkflowId).toHaveBeenCalledWith('wf_research')
