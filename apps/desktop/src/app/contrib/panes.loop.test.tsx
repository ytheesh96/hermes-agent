...
vi.mock('../chat/loop-panel', () => ({
  ...
  loopPanelStateForWorkflow: (
    state: { rows: { workflowId?: string }[]; workflowId?: string; workflowIds: string[] } | null,
    workflowId: string
  ) =>
    state
      ? {
          ...state,
          rows: state.rows.filter(row => (row.workflowId || state.workflowId) === workflowId),
          workflowId,
          workflowIds: [workflowId]
        }
      : null,
  loopWorkflowPaneTitle: (state: { rows: { title: string; workflowId?: string }[] } | null, workflowId: string) =>
    state?.rows.find(row => row.workflowId === workflowId)?.title || workflowId
}))
...
function controller(workflowIds = ['wf-a', 'wf-b']): LoopPanelController {
  return {
    activeWorkflowId: workflowIds[0] || null,
    canvasScopeKey: 'session:one',
    focusedTaskId: null,
    focusRequestKey: 1,
    focusRequestKeysByWorkflow: { 'wf-a': 1, 'wf-b': 1 },
    hidden: false,
...
describe('native Loop workflow panes', () => {
  it('registers one keep-alive native pane per exact scoped workflow id', () => {
    ...
    const paneAId = loopWorkflowPaneId('profile:session:one', 'wf-a')
    const paneBId = loopWorkflowPaneId('profile:session:one', 'wf-b')
  ...
  it('renders and activates only the workflow owned by that native pane', () => {
    ...
    expect(loop.onActivateWorkflowId).toHaveBeenCalledWith('wf-b')
    ...
    expect(mocks.loopPanelProps).toHaveBeenCalledWith(
      expect.not.objectContaining({
        activeWorkflowId: expect.anything(),
        workflowIds: expect.anything()
      })
    )
  })
  ...
  it('routes native tab close to the exact workflow and returns focus after the last one', () => {
    ...
    closeTreePane(loopWorkflowPaneId('profile:session:one', 'wf-a'))
    expect(loop.onCloseWorkflowId).toHaveBeenCalledWith('wf-a')
    expect(mocks.requestComposerFocus).not.toHaveBeenCalled()

    const last = controller(['wf-b'])
    vi.mocked(last.onCloseWorkflowId).mockReturnValue({ closedLast: true, nextWorkflowId: null })
    act(() => $loopPanelController.set(last))

    closeTreePane(loopWorkflowPaneId('profile:session:one', 'wf-b'))
    expect(last.onCloseWorkflowId).toHaveBeenCalledWith('wf-b')
    expect(mocks.requestComposerFocus).toHaveBeenCalledTimes(1)
  })

  it('moves focus to the surviving workflow when closing an inactive singleton split', () => {
    const loop = controller()
    loop.activeWorkflowId = 'wf-a'
    vi.mocked(loop.onCloseWorkflowId).mockReturnValue({ closedLast: false, nextWorkflowId: 'wf-a' })
    act(() => $loopPanelController.set(loop))
    ...
    expect(loop.onCloseWorkflowId).toHaveBeenCalledWith('wf-b')
