...
export function TreeGroup({
  ...
  const activateTabFromKeyboard = (paneId: string) => {
    ...
    const tab = [...(ref.current?.querySelectorAll<HTMLElement>('[data-tree-tab]') || [])].find(
  ...
  useEffect(() => {
    ...
    const activeTab = [...(ref.current?.querySelectorAll<HTMLElement>('[data-tree-tab]') || [])].find(
      tab => tab.dataset.treeTab === activeId
    )
